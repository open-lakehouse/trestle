...
use crate::store::{AssociationStoreReader, Precondition, StoreExec, Transactional};
...
pub async fn cas_update<S: StoreExec<ConformanceLabel>>(store: &S) {
    let obj = store
        .create(ConformanceLabel, &rn("a"), None, None)
        .await
        .unwrap();
...
pub async fn rename_semantics<S: StoreExec<ConformanceLabel>>(store: &S) {
    let a = store
        ...
        .unwrap();
    let b = store
        .create(ConformanceLabel, &rn("b"), None, None)
        .await
        .unwrap();
...
>(
    ...
    let seed = store
        .create(ConformanceLabel, &rn("seed"), None, None)
        .await
        .unwrap();
    let seed_id = seed.id;
    ...
    let res: crate::Result<()> = store
        .transaction(Box::new(move |tx| {
            Box::pin(async move {
                ...
                tx.create(ConformanceLabel, &rn("new"), None, None).await?;
...
>(
    ...
    let res: crate::Result<uuid::Uuid> = store
        .transaction(Box::new(|tx| {
            Box::pin(async move {
                let a = tx.create(ConformanceLabel, &rn("x"), None, None).await?;
                let b = tx.create(ConformanceLabel, &rn("y"), None, None).await?;
...
pub async fn inverse_edges<S: StoreExec<ConformanceLabel>>(store: &S) {
    let p = store
        .create(ConformanceLabel, &rn("p"), None, None)
        .await
        .unwrap();
    let c = store
        .create(ConformanceLabel, &rn("c"), None, None)
        .await
        .unwrap();
