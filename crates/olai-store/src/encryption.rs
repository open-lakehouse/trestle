...
//!
//! Sensitive field values are protected with a standard envelope-encryption scheme:
//!
//! - Each value is encrypted under a fresh, random **data encryption key** (DEK,
...
//!
//! The blob is bound to a caller-supplied `name` as AEAD associated data (AAD), so a ciphertext
//! cannot be silently relocated to a different name. [`ManagedObjectStore`] seals with the
//! object's UUID as the name, binding each sealed blob to the row it belongs to.
//!
//! The default [`KeyProvider`] is [`LocalKeyProvider`], which holds KEK material in process. The
//! [`KeyProvider`] trait is async so external key stores (Azure Key Vault, AWS KMS, GCP KMS,
//! Databricks) can be plugged in later to perform remote wrap/unwrap without changing callers.
//!
//! # Interaction with search
//!
//! `ManagedObjectStore` routes sensitive fields *out* of an object's `properties` payload and
//! seals them separately, so encrypting them does not reduce the searchability of the plaintext
//! `properties`: sensitive values were never part of the searchable payload. Sealed values are
//! non-deterministically encrypted and are not searchable by content — there is no blind indexing.
//!
//! [`ManagedObjectStore`]: crate::ManagedObjectStore
...
pub trait KeyProvider: Send + Sync + 'static {
    ...
    /// # Errors
    ///
    /// Returns an error if `kek_id` is unknown or the underlying cipher fails.
    ...
    /// # Errors
    ///
    /// Returns an error if `kek_id` is unknown, the input is malformed, or authentication fails.
    async fn unwrap_dek(&self, wrapped: &[u8], kek_id: &str) -> Result<Zeroizing<Vec<u8>>>;
}

/// A [`KeyProvider`] that holds KEK material in process.
///
/// Holds one active KEK plus any number of retired KEKs so values sealed under a previous key can
/// still be decrypted and lazily re-wrapped during rotation.
...
impl LocalKeyProvider {
    ...
    /// # Errors
    ///
    /// - [`Error::InvalidArgument`] if any key is not 32 bytes.
    /// - [`Error::InvalidArgument`] if `active` is not among the provided keys.
    pub fn new(
    ...
    /// # Errors
    ///
    /// [`Error::InvalidArgument`] if `key` is not 32 bytes.
    pub fn single(active: impl Into<KekId>, key: Vec<u8>) -> Result<Self> {
        let active = active.into();
        Self::new(active.clone(), [(active, key)])
    }

    /// A fixed, well-known KEK for local development and tests only.
    ///
    /// <div class="warning">
    ///
    /// **Never use this in production.** The key material is a hardcoded constant and provides no
    /// confidentiality. Supply a real KEK (e.g. resolved from an environment variable or KMS) via
    /// [`new`](LocalKeyProvider::new) / [`single`](LocalKeyProvider::single) instead.
    ///
    /// </div>
...
/// High-level envelope-encryption API used to seal and open sensitive values.
///
...
impl EnvelopeEncryptor {
    /// Construct from any [`KeyProvider`].
    ...
    /// Encrypt `plaintext` for `name`, returning a serialized envelope blob.
    ///
    /// `name` is bound as AEAD associated data so the blob cannot be opened under a different name.
    ///
    /// # Errors
    ///
    /// Returns an error if wrapping the DEK or encrypting the value fails.
    ...
    /// Decrypt a serialized envelope `blob` for `name`.
    ///
    /// # Errors
    ///
    /// Returns an error if the blob is malformed, `name` does not match the AAD, the KEK is
    ...
    /// # Errors
    ///
    /// Returns an error if the blob is malformed or the recorded KEK is unavailable.
